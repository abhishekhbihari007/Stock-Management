    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Track all stock movements and changes in real-time</div>
        </div>
      </div>

      {!isLoading && chartData.length > 0 && (
        <div className="stat-card" style={{ marginBottom: '32px', height: '360px', padding: '32px' }}>
          <h3 style={{ marginBottom: '24px', fontSize: '15px', color: 'var(--text)', fontWeight: '600', fontFamily: 'var(--font-heading)' }}>Daily Stock Movement</h3>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="stockIn" name="Stock In" stroke="var(--green)" strokeWidth={3} fillOpacity={1} fill="url(#colorIn)" />
              <Area type="monotone" dataKey="stockOut" name="Stock Out" stroke="var(--red)" strokeWidth={3} fillOpacity={1} fill="url(#colorOut)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="table-container color-amber">
        <div className="table-toolbar">
          ...
          <div className="filter-bar">
            <select className="form-select" style={{ width: '140px' }} value={limit} onChange={e => setLimit(Number(e.target.value))}>
              ...
              <tbody>
                {logs.map(log => (
                  <tr key={log.id}>
                    ...
                    <td style={{ fontFamily: 'var(--font-mono)', color: log.quantity_change > 0 ? 'var(--green)' : 'var(--red)', fontWeight: '700', fontSize: '15px' }}>
                      {log.quantity_change > 0 ? '+' : ''}{log.quantity_change}
                    </td>
                    <td style={{ fontFamily: 'var(--font-mono)', color: 'var(--text3)' }}>{log.quantity_before}</td>
                    <td style={{ fontFamily: 'var(--font-mono)', color: 'var(--text)', fontWeight: '500' }}>{log.quantity_after}</td>
                    <td style={{ maxWidth: '250px', fontSize: '13px', color: 'var(--text2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {log.notes || '—'}
                    </td>
                  </tr>
                ))}
